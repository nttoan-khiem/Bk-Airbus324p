/*
 * TranHoangQuan_Security.c
 *
 * Created: 6/9/2023 3:52:10 PM
 * Author : Nguyen Thanh Toan- Khiem; BK-Airbus Team
 */ 
...
void initialTimer0();             // Init timer use realtime ios   //======HAVE FIXED
void initialLcd();                // Init LCD for userinterface		//=====HACE FIXED
void initialExternalInteruprt();  // Init interrupt for switch lock or unclock		//====HAVE FIXED
void initialPinConfig();          // Config pin			//====HAVE FIXED
void initializationSystem();      // Init system 
void initSPI(unsigned int boudrate);// Init Uart protocol to print report health system		//===HAVE FIXED
...
char testting = 0;
char g_checkLock = 0;
char g_checkUnlock = 0;
//for flag
char g_changeStateLcd = 0;
char g_changeState = 0;
char g_priorityState = 0;
char g_changeSec = 0;
...
void printLcdL();
char checkError();
void USART_TransmitTime();				//HAVE FIXED 
//for lcd data
void setErrorData();
void setNormalData();
//End define software
...
void writeDataLCD(unsigned char data); //function write data to LCD     //=========HAVE FIXED
void writeCommandLCD(unsigned char command); //function write command to LCD	//========HAVE FIXED
void setBitPB(unsigned char bit, char logic); //function to write bit to port B
void setBitPD(unsigned char bit, char logic); //function to write bit to port D
void ledOpen(char input);	//=====HAVE FIXED
void ledFail(char input);	//=====HAVE FIXED
void ledAllow(char input);	//=====HAVE FIXED
void ledDenied(char input); //=====HAVE FIXED
void ledLock(char input);   //=====HAVE FIXED
void ledUnlock(char input); //=====HAVE FIXED
void buzzer(char input);	//=====HAVE FIXED
void soundPushButton();		//=====HAVE FIXED
void USART_Transmit(unsigned char data);	//=======HAVE FIXED
void logicPortA(unsigned char pin, char logic);
void logicPortB(unsigned char pin, char logic);
void logicPortC(unsigned char pin, char logic);
void logicPortD(unsigned char pin, char logic);
//for scankey
unsigned char scanKeyOutside();		//====HAVE FIXED
unsigned char scanKeyInside();		//====HAVE FIXED 
//for testting hardware
unsigned char testKeyPad();			//====HAVE FIXED
unsigned char testDisplay();		//====HAVE FIXED
void testLight();					//====HAVE FIXED
void stateTest();					//====HAVE FIXED
...
char checkCode(){
  ...
  for(counter = 0; counter < 4; counter ++){
    if(g_pinInput[counter] != g_pinCorrect[counter]) return 0;
  }
  return 1;
}
char remainTimeTen(){
...
void writeCommandLCD(unsigned char command){
	...
	logicPortC(0,0);
	PORTD &= 0x0f;
	PORTD |= (0xf0 & (command << 4));
	logicPortC(0,1);
	_delay_ms(2);
	logicPortC(0,0);
}

void setBitPB(unsigned char bit, char logic){
  if(logic){
    PORTB |= 0x01 << bit;
  }else{
    PORTB &= ~(0x01<<bit);
...
void setBitPD(unsigned char bit, char logic){
  if(logic){
    PORTD |= 0x01 << bit;
  }else{
    PORTD &= ~(0x01<<bit);
